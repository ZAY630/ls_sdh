...
get_historical_weather = function(latlongs, 
  ...
  for (station in isds_of_interest) {
    ...
    worldmet::importNOAA(code = station, year = year(begin):year(end), # change date range to suit
                         path = here("climate/weather")) # save hourly data as .rds in 'weather' subfolder (make sure it exists)
    
  }

  if(post_process) {
    # get list of weather files in 'weather' subfolder
    weather_files <- fs::dir_ls(path = here("climate/weather"))
    ...
    write_rds(df_weather, here("climate/weather/weather.rds"), compress = "gz")
    write_csv(df_weather, here("climate/weather/weather.csv"))
  }
